---
title: Webbpush i Adobe Journey Optimizer
description: Lär dig hur du skickar push-meddelanden på webben i Adobe Journey Optimizer
user-guide-title: Skicka push-meddelanden med kampanjer och resor i Adobe Journey Optimizer
user-guide-description: I den här självstudiekursen får du hjälp med att skicka push-meddelanden på webben i Adobe Journey Optimizer med hjälp av kampanjer och Journeys
solution: Journey Optimizer
role: Developer
level: Beginner
breadcrumb-url: /docs/journey-optimizer-learn/ajo-web-push/introduction
source-git-commit: c339fe796af1e691cd3b1c98cd6ba8a8772551e4
workflow-type: tm+mt
source-wordcount: '76'
ht-degree: 1%

---


# Webbpush i Adobe Journey Optimizer{#ajo-web-push}

+ [Introduktion](./introduction.md)
+ [Skapa push-konfiguration](./create-push-configuration.md)
+ [Skapa datastream](./create-data-stream.md)
+ [Lokal installation](./run-application-locally.md)
+ [Hämta användarbehörighet](./capture-user-persmission.md)
+ [Skapa målgrupper](./create-audience.md)
+ [Skapa kampanj](./create-campaign.md)
+ [Skapa taggegenskap](./create-tag-property.md)
+ [Skapa resa](./create-journey.md)
+ [Felsökning](./debugging.md)

